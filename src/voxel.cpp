...
size_t vv::Voxel::getIndex(int x, int y, int z) const {
    return x + N * y + N2 * z;
}

bool vv::Voxel::checkNpyArray(const cnpy::NpyArray& array) {
...
bool vv::Voxel::checkRange(int coord) const {
    return coord >= 0 && coord <= N;
}

vv::Voxel::Voxel(const size_t n, cnpy::NpyArray voxel)
        : N(n), N2(n * n), mOccupy(n * n * n, false) {
...
std::string vv::Voxel::toObj() const {
